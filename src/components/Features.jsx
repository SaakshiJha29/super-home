...
export default function Features() {
  const features = [
    {
      ...
      glow: 'from-sky-200/40 to-sky-300/10',
  ...
  schema: './user-schema.pql'
    ...
    },
    {
      ...
      glow: 'from-amber-200/40 to-amber-300/10',
      accentColor: 'text-amber-600',
      borderColor: 'hover:border-amber-400',
      widget: (
        <div className="bg-amber-50/80 rounded-xl p-3.5 border border-amber-200/80 space-y-2 text-xs font-mono">
          <div className="flex justify-between items-center text-slate-800">
            <span className="flex items-center gap-1.5 text-amber-700 font-bold">
              <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
              Fault Profile Active
            </span>
            <span className="text-[10px] text-amber-800 font-semibold">15% Drop Rate</span>
          </div>
    ...
    },
    {
      ...
      icon: Globe,
      badge: '35+ Regions',
      glow: 'from-sky-300/30 to-indigo-200/20',
  ...
  return (
    <section id="features" className="py-24 bg-mesh-light relative overflow-hidden border-t border-sky-100">
      
      {/* Background ambient lighting */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-sky-200/30 rounded-full blur-[140px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="w-12 h-12 rounded-2xl bg-sky-50 border border-sky-100 flex items-center justify-center group-hover:scale-105 transition-transform shadow-sm">
                      <IconComponent className={`w-6 h-6 ${feature.accentColor}`} />
                    </div>
                    <span className="text-[11px] font-mono font-bold px-2.5 py-1 rounded-full bg-slate-100/90 text-slate-700 border border-slate-200">
                      {feature.badge}
                    </span>
                  </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-200/80">
                  {feature.codeSnippet && (
                    <div className="bg-[#0b1326] rounded-xl p-3.5 border border-slate-800 text-[11px] font-mono text-slate-200 overflow-x-auto shadow-sm">
                  ...
                  {feature.nodes && (
                    <div className="flex flex-wrap gap-2 pt-1">
                      {feature.nodes.map((node) => (
                        <span key={node} className="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-[11px] font-mono text-slate-700 font-semibold shadow-sm flex items-center gap-1.5">
        ...
        {/* Bottom Specs Note */}
        <div className="mt-12 glass-panel p-6 rounded-2xl border border-sky-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-slate-900">Full Protocol Parity</h4>
              <p className="text-xs text-slate-600">Supports OpenAPI 3.0, GraphQL Schema Spec, gRPC reflections, and WebSockets.</p>
            </div>
          </div>
          <a
            href="#sandbox"
            className="px-5 py-2.5 rounded-xl bg-sky-50 hover:bg-sky-100 border border-sky-200 text-sky-700 font-bold text-xs font-mono transition-colors shrink-0 flex items-center gap-1"
          >
